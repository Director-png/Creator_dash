...
def load_market_pulse_data():
    try:
        ...
        if 'Niche' not in df.columns: return pd.DataFrame()
        ...
        with col_mid:
            ...
            if st.button("Access System", use_container_width=True):
                ...
                if email_in == "admin" and pw_in == "1234":
                    ...
                    st.session_state.user_name = "Master Director"; st.session_state.user_role = "admin"
                    st.rerun()
    ...
    with t2:
        ...
        with col_mid:
            with st.form("reg_form"):
                name = st.text_input("Name"); email = st.text_input("Email"); niche = st.text_input("Niche"); pw = st.text_input("Password", type="password")
                if st.form_submit_button("Submit Registration"):
                    payload = {"entry.483203499": name, "entry.1873870532": email, "entry.1906780868": niche, "entry.1396549807": pw}
                    try: requests.post(FORM_POST_URL, data=payload); st.success("Transmitted!")
...
if nav == "📊 Dashboard":
    ...
    m2.metric(label="Scripts Ready", value=str(len(st.session_state.script_history)), delta="+6")
    ...
    with col_left:
        ...
        df_road = pd.DataFrame({"Phase": ["VOID Intelligence", "Script Architect", "Client Pitcher", "Agency Portal"], "Status": ["Stable", "Stable", "Online", "Planned"], "Priority": ["Completed", "Active", "High", "Critical"]})
        st.table(df_road)
    with col_right:
    ...
    with tab_p:
        if not st.session_state.pitch_history: st.write("No pitches logged.")
        for p in reversed(st.session_state.pitch_history):
            with st.expander(f"🕒 {p['time']} - Target: {p['client']}"): st.write(p['pitch'])
    with tab_s:
        if not st.session_state.script_history: st.write("No scripts logged.")
        for s in reversed(st.session_state.script_history):
            with st.expander(f"🕒 {s['time']} - Topic: {s['topic']}"): st.write(s['script'])
    ...
    if not data.empty:
        ...
        fig = px.bar(chart_data, x=chart_data.columns[1], y=chart_data.columns[0], orientation='h', color_continuous_scale='Blues', template="plotly_dark")
        st.plotly_chart(fig, use_container_width=True)

elif nav == "⚔️ Trend Duel":
    st.title("⚔️ COMPETITIVE INTELLIGENCE MATRIX")
    pulse_df = load_market_pulse_data()
    if pulse_df.empty: st.warning("VOID Sheet is empty.")
    else:
        niche_list = pulse_df['Niche'].unique().tolist()
        selected_niches = st.multiselect("Select Niches", options=niche_list, default=niche_list[:5])
    ...
    with c1:
        ...
        niche_category = st.selectbox("Category Type", ["Personal Brand (Fitness/Mindset)", "B2B Technical (AI/SaaS)", "Fashion & Luxury (Apparel/E-com)", "Hospitality (Restaurants/Cafes)", "Local Business (Real Estate/Gyms)"])
        ...
        if pitch_btn and client_name:
            with st.spinner("🌑 ACCESSING VOID COGNITION..."):
                try:
                    ...
                    full_prompt = f"Write a pitch for {client_name}. Niche: {niche_category}. Offer: {offer}"
                    res = groq_client.chat.completions.create(model="llama-3.3-70b-versatile", messages=[{"role": "system", "content": system_prompt}, {"role": "user", "content": full_prompt}])
